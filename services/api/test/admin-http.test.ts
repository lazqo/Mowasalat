...
test("ops endpoints refuse everyone without a token", async () => {
  await withApi(async (base) => {
    assert.equal((await fetch(`${base}/admin/routes`)).status, 401);
    assert.equal((await fetch(`${base}/admin/drivers`)).status, 401);
    assert.equal((await post(base, `/admin/routes/${ROUTE}/width`, { widthM: 900 })).status, 401);
  });
});

test("a wrong token is refused", async () => {
  await withApi(async (base) => {
    const res = await fetch(`${base}/admin/routes`, { headers: { authorization: "Bearer wrong" } });
...
test("a token of the right length but wrong content is refused", async () => {
  await withApi(async (base) => {
    ...
    const res = await fetch(`${base}/admin/routes`, { headers: { authorization: `Bearer ${nearly}` } });
    assert.equal(res.status, 401);
  });
});
...
test("with no token configured the ops endpoints do not serve at all", async () => {
  ...
  await withApi(async (base) => {
    assert.equal((await fetch(`${base}/admin/routes`)).status, 503);
    assert.equal((await fetch(`${base}/admin`)).status, 503);
  }, { withAdmin: false });
});

test("public endpoints keep working when ops is disabled", async () => {
  await withApi(async (base) => {
    assert.equal((await fetch(`${base}/health`)).status, 200);
    assert.equal((await post(base, "/trips", { routeId: ROUTE, dir: 0 })).status, 200);
  }, { withAdmin: false });
});

test("an authorised operator lists the five lines", async () => {
  await withApi(async (base, auth) => {
    const body = await (await fetch(`${base}/admin/routes`, { headers: auth })).json();
...
test("an edit that would break a line is refused with the reasons", async () => {
  await withApi(async (base, auth) => {
    const res = await post(base, `/admin/routes/${ROUTE}/width`, { widthM: 10 }, auth);
...
test("a valid width edit is saved", async () => {
  await withApi(async (base, auth) => {
    const res = await post(base, `/admin/routes/${ROUTE}/width`, { widthM: 1_100 }, auth);
    assert.equal(res.status, 200);
    const after = await (await fetch(`${base}/admin/routes/${ROUTE}`, { headers: auth })).json();
...
test("an unknown line returns 404", async () => {
  await withApi(async (base, auth) => {
    assert.equal((await fetch(`${base}/admin/routes/jo-nowhere`, { headers: auth })).status, 404);
  });
});
...
test("traces suggest a width without applying it, then apply on request", async () => {
  await withApi(async (base, auth) => {
    ...
    const dry = await (await post(base, `/admin/routes/${ROUTE}/traces`, { traces: [drive] }, auth)).json();
    assert.ok(dry.suggestion.widthM > 0);

    const before = await (await fetch(`${base}/admin/routes/${ROUTE}`, { headers: auth })).json();
    assert.equal(before.provisional, true, "still untouched by the dry run");

    const applied = await (await post(base, `/admin/routes/${ROUTE}/traces`, { traces: [drive], apply: true }, auth)).json();
...
test("the driver roster runs end to end over the wire", async () => {
  await withApi(async (base, auth) => {
    const driver = await (await post(base, "/admin/drivers", { phone: "+962790000001" }, auth)).json();
    assert.equal(driver.tier, 0);
    assert.equal(driver.phoneMasked, "••• 001");

    await post(base, `/admin/drivers/${driver.id}/routes`, { routeId: ROUTE }, auth);
    const vouched = await (await post(base, `/admin/drivers/${driver.id}/vouch`, { authority: "drivers_committee" }, auth)).json();
    assert.equal(vouched.tier, 1);

    const listed = await (await fetch(`${base}/admin/drivers`, { headers: auth })).json();
...
test("a phone number never comes back over the wire", async () => {
  await withApi(async (base, auth) => {
    await post(base, "/admin/drivers", { phone: "+962790000001" }, auth);
    const body = await (await fetch(`${base}/admin/drivers`, { headers: auth })).text();
...
test("an unrecognised vouching authority is refused", async () => {
  await withApi(async (base, auth) => {
    const driver = await (await post(base, "/admin/drivers", { phone: "+962790000002" }, auth)).json();
    const res = await post(base, `/admin/drivers/${driver.id}/vouch`, { authority: "some_guy" }, auth);
...
test("a driver id never appears in anything a passenger can see", async () => {
  ...
  await withApi(async (base, auth) => {
    const driver = await (await post(base, "/admin/drivers", { phone: "+962790000003" }, auth)).json();
    ...
    const seen = await (await post(base, "/buses", { routeId: ROUTE, dir: 0, remainingM: 5_000, zoneSeq: 0 })).text();
